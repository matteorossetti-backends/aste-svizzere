...
def place_bid(auction_id):
    ...
    return jsonify({"message": f"{bidder} is now winning!", "item": auction["item"], "bid": f"{amount} CHF"}), 200
...
def get_winner(auction_id):
    ...
    return jsonify({"item": auction["item"], "winner": auction["winner"], "bid": f"{auction['current_bid']} CHF"})
